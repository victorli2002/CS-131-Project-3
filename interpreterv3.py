...
class Interpreter(InterpreterBase):
  ...
  def _assign(self, tokens):
   ...
   check = vname.split(".")
   if len(check) == 2: # for object member assignments
    self.set_object_member(check[0], check[1], value_type)
    self._advance_to_next_statement()
    return
   elif len(check) > 2:
    super().error(ErrorType.SYNTAX_ERROR,"Too many dots lol")
   ...
   if existing_value_type.type() != value_type.type():
     super().error(ErrorType.TYPE_ERROR,
                   ...
                   self.ip)
   if value_type.type() == Type.OBJECT: #object reference
  ...
  def set_object_member(self, object, member, value_type):
    object_dict = self.get_object_dict(object)
    object_dict[member] = value_type
    return
  
  def get_object_member(self, object, member):
    object_dict = self.get_object_dict(object)
    if member not in object_dict:
      super().error(ErrorType.NAME_ERROR, f"member variable {member} does not exist in {object}", self.ip)
    return object_dict[member]

  def get_object_dict(self, object):
    ob = self.env_manager.get(object)
    if ob == None:
      super().error(ErrorType.NAME_ERROR, f"object {object} does not exist", self.ip)
    if ob.type() != Type.OBJECT:
      super().error(ErrorType.TYPE_ERROR, f"var {object} is not an object", self.ip) 
    return ob.value()
  ...
  def _funccall(self, args):
    ...
    if args[0] == InterpreterBase.PRINT_DEF:
      ...
      elif value_type.type() != Type.FUNCVAR:
        super().error(ErrorType.TYPE_ERROR,"Must call function using function variable", self.ip)
      else:
        func_name = value_type.value()
        if func_name == None: #even if we assigned a method to default, we wouldn't need to worry about a this keyword being used
          if len(args) > 1:
            super().error(ErrorType.NAME_ERROR,f"Mismatched parameter count in call to default function", self.ip)
          self._advance_to_next_statement()
        else:
          this = False # to check for dot notation and this
          object_check = args[0].split(".")
          if len(object_check) == 2: # dot notation
            this = True
          if len(func_name.split(":")) > 1:
            print("LAMBDA")
          self.return_stack.append(self.ip+1)
          self._create_new_environment(func_name, args[1:], self._get_value(object_check[0]), this)
          self.ip = self._find_first_instruction(func_name)

  # create a new environment for a function call
  def _create_new_environment(self, funcname, args, object, this = False):
  ...
  def _setup_default_values(self):
    ...
    # TODO: implement the dummy function stuff
    self.type_to_default[InterpreterBase.FUNC_DEF] = Value(Type.FUNCVAR,None)
    ...
    self.compatible_types[InterpreterBase.FUNC_DEF] = Type.FUNCVAR
    ...
    self.type_to_result[Type.FUNCVAR] = 'f'
  ...
  def _setup_operations(self):
    ...
    self.binary_ops[Type.OBJECT] = {
     '==': lambda a,b: Value(Type.BOOL, a.value()==b.value()),
    }
    self.binary_ops[Type.FUNCVAR] = {
      '==': lambda a,b: Value(Type.BOOL, a.value()==b.value()),
    }
  ...
  def _get_value(self, token):
    ...
    #to check if it's a function name
    if self.func_manager.get_function_info(token) != None:
      return Value(Type.FUNCVAR, token)
    check = token.split(".")
    if len(check) == 2: # for object member access
      return self.get_object_member(check[0], check[1])
    elif len(check) > 2:
      super().error(ErrorType.SYNTAX_ERROR,"Too many dots lol")
